...
export class EditHrpDetailsPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.hrpService.getHrp(this.hrpId).subscribe((response) => {
  ...
  loadMap() {
    ...
    if (!this.hrpToEdit.latitude) {
      ...
      let marker: Marker = this.map.addMarkerSync({
        position: new LatLng(this.hrpToEdit.latitude, this.hrpToEdit.longitude)
      });
  ...
  async editHrp(hrpForm: NgForm) {
    if (hrpForm.valid) {
      ...
      this.hrpService.updateHrp(this.hrpToEdit).subscribe(async (response) => {
        const toast = await this.toastController.create({
